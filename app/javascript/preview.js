document.addEventListener('DOMContentLoaded', function(){
  //商品出品、編集ページのフォームを取得
  const itemForm = document.getElementById('item_box');
  //プレビューを表示するためのスペースを取得
  const previewList = document.getElementById('previews');
  //商品出品、編集ページのフォームがないならここで終了
  ...
  const changedFileField = (e) => {
    ...
    //fileが空 = 何も選択しなかったらプレビュー等を削除して終了する
    if (!file) {
    ...
    //data-indexを使用して、既にプレビューが表示されているか確認する
    const alreadyPreview = document.querySelector(`.preview[data-index="${dataIndex}"]`);

    if (alreadyPreview) {
      //クリックしたfile_fieldのdata-indexと同じ番号のプレビュー画像が既に表示されている場合は、画像の差し替えのみを行う
